...
class StripHeaderSpaces(Setting):
    ...
    desc = """\
        ...
        """
# --- CONFIGATTRIBUTE_STUB_START ---
# Stub minimale per compat con codice che importa "ConfigAttribute" da config.py
try:
    ConfigAttribute  # se già definito, non fare nulla
except NameError:
    class ConfigAttribute:
        ...
        def __get__(self, obj, owner=None):
            if obj is None:
                return self
            src = getattr(obj, "config", None)
            val = (src.get(self.name) if hasattr(src, "get")
                   else getattr(obj, self.name, None))
            return self.get_converter(val) if (self.get_converter and val is not None) else val
        def __set__(self, obj, value):
            src = getattr(obj, "config", None)
            if hasattr(src, "__setitem__"):
                src[self.name] = value
            else:
                setattr(obj, self.name, value)
# --- CONFIGATTRIBUTE_STUB_END ---
