...
CATALOGHI = [{"id":"CAT-001","nome":"Imbracature"}, {"id":"CAT-002","nome":"Cordini"}]
...
def _csv_response(rows, filename):
    ...
    w = csv.DictWriter(buf, fieldnames=rows[0].keys()) if rows else csv.DictWriter(buf, fieldnames=["vuoto"])
    w.writeheader()
...
def export_cataloghi():   return _csv_response(CATALOGHI, "cataloghi.csv")

@router.get("/schede/export")
def export_schede():      return _csv_response(SCHEDE, "schede.csv")

@router.get("/percorsi/export")
def export_percorsi():    return _csv_response(PERCORSI, "percorsi.csv")

@router.post("/cataloghi/import")
async def import_cataloghi(file: UploadFile = File(...)):
    if not file.filename.lower().endswith(".csv"): raise HTTPException(400, "Carica un CSV")
    text = (await file.read()).decode("utf-8-sig")
    rdr = csv.DictReader(io.StringIO(text))
    rows = list(rdr)
    if rows:  # replace mock for now
        global CATALOGHI; CATALOGHI = rows
