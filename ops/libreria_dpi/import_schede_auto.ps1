param(
  ...
  [string[]]$SourceDirs = @(
    "C:\Users\G200\OneDrive\Desktop\schede\ikar",
    "C:\Users\G200\OneDrive\Desktop\schede\teuf"
  ),
  [ValidateSet("COPY","MOVE")] [string]$Action = "COPY",
  # B = duplicati hash spostati in _DUPLICATI_HASH (pulizia)
  [ValidateSet("COPY","MOVE")] [string]$DupAction = "MOVE",
...
$LogPath = Join-Path $LogDir ("import_auto_{0}_{1}.csv" -f (Get-Date -Format "yyyy-MM-dd_HHmmss"), $Action)
...
function Guess-Produttore([string]$path){
  $p = $path.ToUpper()
  if($p -match '\\IKAR\\' -or $p -match '\bIKAR\b'){ return "IKAR" }
  if($p -match '\\TEUF\\' -or $p -match '\bTEUF\b' -or $p -match '\bTEUFELBERGER\b'){ return "TEUFELBERGER" }
  return ""
}
...
function Guess-Modello([string]$path){
  ...
  # --- IKAR: HWB/HWPB/HRA/HRAE con virgola/punto (1,8 -> 18)
  ...
  # --- IKAR: DB-A2
  if($n0 -match '\bDB\s*[-_ ]*A2\b'){
    return "DB_A2"
  }

  # --- IKAR: ABS 3A + varianti (WHSL/WHPL/WH/W)
  if($n0 -match '\bABS\s*3A\b'){
    $suffix = ""
    if($n0 -match '\bWHSL\b'){ $suffix = $suffix + "_WHSL" }
    if($n0 -match '\bWHPL\b'){ $suffix = $suffix + "_WHPL" }
    if($n0 -match '(\bWH\b|_WH_)'){ $suffix = $suffix + "_WH" }
    if($n0 -match '(\bW\b|_W_)'){ $suffix = $suffix + "_W" }
...
function Guess-Categoria([string]$tipo,[string]$prod,[string]$mod,[string]$path){
  # retrattili
...
Write-Host ""
Write-Host "=== AUTO IMPORT SCHEDE (DEDUP HASH -> _DUPLICATI_HASH) ==="
Write-Host ("ROOT: " + $Root)
Write-Host ("Action: " + $Action + " | DupAction: " + $DupAction + " | VersioneData: " + $VersioneData)
Write-Host ("LOG: " + $LogPath)
Write-Host ""

[int]$ok=0; [int]$rev=0; [int]$dup=0; [int]$err=0

foreach($sd in $SourceDirs){
  if(!(Test-Path $sd)){ Write-Host ("SKIP (missing): " + $sd); continue }

  $pdfs = Get-ChildItem -LiteralPath $sd -Recurse -File -Filter *.pdf -ErrorAction SilentlyContinue
  foreach($f in $pdfs){
    try{
      # 0) HASH DEDUP (prima di tutto)
      $srcHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $f.FullName).Hash
      if($KnownHashes.ContainsKey($srcHash)){
        $dupDest = Safe-Dest (Join-Path $DupHashDir $f.Name)
        CopyOrMove $f.FullName $dupDest $DupAction
        $dup++
        $note = "DUP_HASH of: " + $KnownHashes[$srcHash]
        Add-Content -Encoding UTF8 $LogPath (CsvLine @(
          (Get-Date -Format "yyyy-MM-dd HH:mm:ss"),"SKIP_DUP_HASH","","","","",$VersioneData,$f.FullName,$dupDest,$srcHash,$note
        ))
        continue
      }

      # 1) guess metadata
      $tipo = Guess-Tipo $f.FullName
      $prod = Guess-Produttore $f.FullName
      $mod  = Guess-Modello $f.FullName
      $cat  = Guess-Categoria $tipo $prod $mod $f.FullName

      # 2) se manca categoria o modello -> review
      if([string]::IsNullOrWhiteSpace($cat) -or [string]::IsNullOrWhiteSpace($mod)){
        $dest = Safe-Dest (Join-Path $InboxReview $f.Name)
        CopyOrMove $f.FullName $dest $Action
        $rev++
        Add-Content -Encoding UTF8 $LogPath (CsvLine @(
          (Get-Date -Format "yyyy-MM-dd HH:mm:ss"),"NEEDS_REVIEW",$tipo,$cat,$prod,$mod,$VersioneData,$f.FullName,$dest,$srcHash,"AUTO: manca categoria/modello"
        ))
        continue
      }
      ...
      # 3) importa
      if($tipo -eq "CE"){
        $res = Add-CE $f.FullName $cat $prod $mod $VersioneData ""
      } else {
        $res = Add-Manual $f.FullName $cat $prod $mod $VersioneData ""
      }
      ...
      Add-Content -Encoding UTF8 $LogPath (CsvLine @(
        (Get-Date -Format "yyyy-MM-dd HH:mm:ss"),"OK",$tipo,$cat,$prod,$mod,$VersioneData,$f.FullName,$dest,$hash,""
      ))
      $ok++
    }
    catch{
...
Write-Host ""
Write-Host ("OK: {0} | NEEDS_REVIEW: {1} | DUP_HASH: {2} | ERROR: {3}" -f $ok,$rev,$dup,$err)
Write-Host ("Review folder: " + $InboxReview)
Write-Host ("Dup folder   : " + $DupHashDir)
Write-Host ("Log          : " + $LogPath)
Write-Host "DONE."
