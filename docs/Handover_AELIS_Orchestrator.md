...
import sicuro via FastAPI, dedup globale, audit completo e CI verde. Preparare la ‚¬Å“fase comandi‚¬.
...
3) Colonne extra escluse dal ‚¬Å“clean‚¬; preservate in extras.csv o report.columns_extra.
4) .xlsm/macros: rifiuto (MACRO_DETECTED 422) salvo allow_macros+scan; mai eseguire macro.
5) ZIP/streaming: limiti (50 file, 200 MB tot, 5 MB singolo), zip-slip defense (niente symlink/‚¬Å“..‚¬).
...
8) OsservabilitÃƒ  JSONL + X-Correlation-ID; no PII nei log; retry/backoff (no 4xx).
